    ...
    async def _proceedToClone(self):
        if (is_gdrive_link(self.link) or is_gdrive_id(self.link)) and is_gdrive_id(self.upDest):
            ...
            if is_gdrive_link(self.link):
                ...
                cmd = ['huntclone', 'lsjson', '--fast-list', '--stat', '--no-modtime', '--config', config_path, f'{remote}:{src_path}']
            ...
            LOGGER.info('Cloning Done: %s', self.name)
            cmd1 = ['huntclone', 'lsf', '--fast-list', '-R', '--files-only', '--config', config_path, destination]
            cmd2 = ['huntclone', 'lsf', '--fast-list', '-R', '--dirs-only', '--config', config_path, destination]
            cmd3 = ['huntclone', 'size', '--fast-list', '--json', '--config', config_path, destination]
