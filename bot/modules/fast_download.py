...
class FastDL(TaskListener):
    ...
    async def newEvent(self):
        ...
        rjson = await cmd_exec(['huntclone', 'backend', 'addurl', '--config', 'rclone.conf', upload_path, self.link])
        if rjson[2] != 0:
        ...
        if rjson.get('message') == 'Saving':
            await sleep(4)
            text += ('<bStatus:</b> Complete\n'
                     f'<b>Size:</b> {get_readable_file_size(rjson.get("file_size"))}\n')
            url = f'{config_dict["RCLONE_SERVE_URL"]}/{url_path}/{rutils.quote(name)}'
            typee = await cmd_exec(['huntclone', 'lsjson', '--fast-list', '--stat', '--no-modtime', '--config', 'rclone.conf', f'{upload_path}/{name}'])
            res = loads(typee[0])
