        ...
        if (saved_content := cur_content.get('data')) and cur_content.get('mode') == self.mode:
            ...
            if self.mode == 'telegram':
                ...
                for remote in config.sections():
                    ...
                    cmd = ['huntclone', 'lsjson', typee, '--fast-list', '--no-modtime', '--ignore-case', '-R', '--include', f'*{self.query}*', '--config', self.config_path, f'{remote}:']
                        ...
                        if files:
                            ...
                            for file in files:
                                ...
                                if isdir and self.query.lower() not in name.lower():
                                    continue
                                cmd = ['huntclone', 'link',  '--config', self.config_path, f'{remote}:{file["Path"]}']
