...
async def get_user_settings(from_user, data: str, uset_data: str):
    ...
    if not data:
        sendpm, buttonkey = (' ✓', '✓ Bot PM') if user_dict.get('enable_pm') else (' ✘', 'Bot PM')
        ...
        premsg, buttonkey = (prename, ' ✓ Leech Prefix') if (prename := user_dict.get('prename')) else  (' ✘', 'Leech Prefix')
        buttons.button_data(buttonkey, f'userset {user_id} setdata prename')

        sufmsg, buttonkey = (sufname, ' ✓ Leech Suffix') if (sufname := user_dict.get('sufname')) else (' ✘', 'Leech Suffix ')
        ...
        dumpch, buttonkey = (f'<code>{user_dict.get("dump_ch")}</code>', ' ✓ Dump') if user_dict.get('dump_ch') else ('<b> ✘</b>', 'Dump')
        buttons.button_data(buttonkey, f'userset {user_id} setdata dump_ch')

        gdxmsg, buttonkey = (' ✓', ' ✓ GDrive Tools') if await aiopath.exists(token_pickle) else (' ✘', 'GDrive Tools')
        ...
        if config_dict['PREMIUM_MODE']:
            if (user_premi := is_premium_user(user_id)) and (time_data := user_dict.get('premium_left')):
                if time_data - time() <= 0:
                    ...
                    premium_left = f'<b>Premium Left: </b>{get_readable_time(time_data - time())}</b>\n'
            if user_id != config_dict['OWNER_ID']:
                status_user = '<b> Status: </b>PREMIUM\n' if user_premi else '<b>Status:</b> NORMAL\n'
        if config_dict['DAILY_MODE'] and not is_premium_user(user_id):
            await UserDaily(user_id).get_daily_limit()
            daily_limit = f'<b>Daily Limit: </b>{get_readable_file_size(user_data[user_id]["daily_limit"])}/{config_dict["DAILY_LIMIT_SIZE"]}GB\n'
            daily_limit += f'<b>Reset Time: </b>{get_readable_time(user_data[user_id]["reset_limit"] - time())}\n'
        text = ('<b>User Setttings</b>\n\n'
                f'Settings For: <b>{from_user.mention}</b>\n'
                f'{status_user}'
                f'{premium_left}'
                f'{daily_limit}'
                f'<b>Leech Type: </b>{ltype}\n'
                f'<b>As Group: </b>{mediagroup}\n'
                f'<b>Thumbnail: </b>{thumbmsg}\n'
                f'<b>DM Mode: </b>{sendpm}\n'
        ...
        rccmsg, buttonkey = (' ✓', ' ✓ RClone Config') if await aiopath.exists(rclone_path) else (' ✘', 'RClone Config')
        buttons.button_data(buttonkey, f'userset {user_id} setdata rclone_config')

        rcpathmsg, buttonkey = (f'<code>{rc_path}</code>', ' ✓ RClone Path') if (rc_path := user_dict.get('rclone_path')) else ('<b> ✘</b>', 'RClone Path')
        ...
        gdrive_id, buttonkey = (f'<code>{gd_id}</code>', ' ✓ Drive ID') if (gd_id := user_dict.get('gdrive_id')) else ('<b> ✘</b>', 'Drive ID')
        buttons.button_data(buttonkey, f'userset {user_id} setdata gdrive_id')

        index_url, buttonkey = (f'<code>{index}</code>', ' ✓ Index URL') if (index := user_dict.get('index_url')) else ('<b> ✘</b>', 'Index URL')
        buttons.button_data(buttonkey, f'userset {user_id} setdata index_url')

        token_pickle, buttonkey = (' ✓', ' ✓ Token Pickle') if await aiopath.exists(token_pickle) else (' ✘', 'Token Pickle')
        ...
        stop_dup_msg, buttonkey = (' ✓', ' ✓ Stop Duplicate') if stop_dup else (' ✘', 'Stop Duplicate')
        buttons.button_data(buttonkey, f'userset {user_id} stop_duplicate {stop_dup}', 'header')

        if await aiopath.exists('accounts'):
            use_sa, buttonkey = (' ✓', ' ✓ Use SA') if user_dict.get('use_sa') else (' ✘', 'Use SA')
...
async def edit_user_settings(client: Client, query: CallbackQuery):
    ...
    if config_dict['PREMIUM_MODE'] and not is_premium_user(user_id) and pre_data in premi_features:
        await query.answer('Upss, Premium User Required!\n Contact To Premium: @AmbassadorXD', True)
        is_modified = False
...
async def set_premium_users(_, message: Message):
    ...
    if reply_to and len(args) > 1:
        ...
        for id_, value in user_data.items():
            if value.get('is_premium') and (time_left := value['premium_left']) - time() > 1:
                text += f'{i}. @{value.get("username")} (<code>{id_}</code>) ~ {get_readable_time(time_left - time())}\n'
                i += 1
    else:
        await sendMessage(text, message)
        return
    ...
    if args[1] == 'add':
        ...
        text = f'😘 Yeay, <b>{premi_id}</b> has been added as <b>Premium User</b> for {day} days.'
        user_text = f'Yeay 😘, you have been added as <b>Premium User</b> for {day} days.'
