...
def get_readable_timee(seconds, only_hr_min=False):
    seconds = int(seconds)
    periods = [('d', 86400), ('h', 3600), ('m', 60), ('s', 1)]
    string = ''
    remain = seconds
    for period_name, period_seconds in periods:
        if remain >= period_seconds:
            value, remain = divmod(remain, period_seconds)
            string += f"{int(value)}{period_name} "
            if only_hr_min and period_name == 'm':
                break
    string = string.strip()
    if only_hr_min:
...
def get_readable_message(sid: int, is_user: bool, page_no: int=1, status : str='All', page_step: int=1):
    msg = f'<a href="https://t.me/TG_DC_BoTs"><b><i>Powered By DC BoTs</b></i></a>\n'
    dl_speed = up_speed = 0
    ...
    buttons = ButtonMaker()
    if not is_user:
        buttons.button_data('🎛️', 'status 0 ov', 'header')
    ...
    buttons.button_data('Ref', f'status {sid} ref', 'header')
    if is_user:
