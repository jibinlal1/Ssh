...
class TaskConfig:
    ...
    # ┎┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┒
    # ┃ NEW FUNCTION TO REMOVE WWW PREFIX                                  ┃
    # ┖┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┚
    async def remove_www_prefix(self, dl_path: str):
        """
        Recursively removes 'www.site.com - ' prefixes from files and directories.
        """
        # Regex to find prefixes like "www.anything.com - "
        www_prefix_regex = re.compile(r'^www\..*?\s-\s')

        async def rename_item(current_path):
            basename = ospath.basename(current_path)
            if www_prefix_regex.match(basename):
                new_basename = www_prefix_regex.sub('', basename)
                new_path = ospath.join(ospath.dirname(current_path), new_basename)
                await aiorename(current_path, new_path)
                return new_path
            return current_path

        if await aiopath.isdir(dl_path):
            # Walk through the directory from bottom up to rename files before directories
            for dirpath, _, filenames in await sync_to_async(walk, dl_path, topdown=False):
                for filename in filenames:
                    file_path = ospath.join(dirpath, filename)
                    await rename_item(file_path)
            # Finally, rename the root directory itself if needed
            new_dl_path = await rename_item(dl_path)
            return new_dl_path
        elif await aiopath.isfile(dl_path):
            return await rename_item(dl_path)
