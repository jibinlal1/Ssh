...
async def add_rclone_download(listener: task.TaskListener, path: str):
    ...
    cmd1 = ['huntclone', 'lsjson', '--fast-list', '--stat', '--no-mimetype', '--no-modtime', '--config', config_path, f'{remote}:{listener.link}']
    cmd2 = ['huntclone', 'size', '--fast-list', '--json', '--config', config_path, f'{remote}:{listener.link}']
