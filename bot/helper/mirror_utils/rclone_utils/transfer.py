...
class RcloneTransferHelper:
    ...
    async def _get_gdrive_link(self, config_path, remote, rc_path, mime_type):
        if mime_type == 'Folder':
            ...
            epath = f'{remote}:{rc_path}{self._listener.name}'
            destination = epath
        cmd = ['huntclone', 'lsjson', '--fast-list', '--no-mimetype', '--no-modtime', '--config', config_path, epath]
    ...
    async def upload(self, path, size):
        ...
        if remote_type == 'drive':
            ...
            cmd = ['huntclone', 'link', '--config', oconfig_path, destination]
            res, err, code = await cmd_exec(cmd)
            if code == 0:
                link = res
            elif code != -9:
    ...
    async def clone(self, config_path, src_remote, src_path, mime_type, drive_id):
        ...
        cmd = ['huntclone', 'link', '--config', config_path, destination]
        res, err, code = await cmd_exec(cmd)
        if self._is_cancelled:
            return None, None
        if code == 0:
    ...
    def _getUpdatedCommand(self, config_path, source, destination, method):
        ...
        cmd = ['huntclone', '--fast-list', '--config', config_path, '-P']
