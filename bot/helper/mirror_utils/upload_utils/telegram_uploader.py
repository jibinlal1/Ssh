...
from bot.helper.hunt_utils.caption_gen import generate_caption
...
class TgUploader:
    ...
    async def _prepare_file(self, file_, dirpath):
        caption = await self._caption_mode(file_, dirpath)
        if len(file_) > 60:
            if is_archive(file_):
    ...
    async def _caption_mode(self, file, dirpath):
        if self._user_caption:
            caption = await generate_caption(file, dirpath, self._user_caption)
        else:
